...
export const getProducts =
  ...
  async (dispatch) => {
    try {
      ...
      let link = `${process.env.REACT_APP_BACKEND_URL}/products?keyword=${keyword}&page=${currentPage}&price[gte]=${price[0]}&price[lte]=${price[1]}&rating[gte]=${ratings}`;

      if (category && category !== "All") {
        link = `${process.env.REACT_APP_BACKEND_URL}/products?keyword=${keyword}&page=${currentPage}&price[gte]=${price[0]}&price[lte]=${price[1]}&category=${category}&rating[gte]=${ratings}`;
      }
...
export const getAllReviews = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/reviews?id=${id}`,{
      withCredentials: true,
    });
